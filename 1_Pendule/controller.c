...
void init_control(float pposition0,float pposition90,float porigin, float pangle_15, float pangle15, float pangle0){
	...
	angle_15=pangle_15;
	angle15=pangle15;
	pasPosition=(position0-position90)/POSITION_WIDTH;
	printk(" pasPosition= %d\n",(int)(pasPosition*1000));  //Volt/cm
	
    pasAngle= (angle15-angle_15)/ANGLE_WIDTH;          //volt/degre
    printk(" pasAngle= %d\n",(int)(pasAngle*1000));  //Volt/cm
}
...
/*	x1 = +0.6300*x1k +0.0261*x2k -0.0012*x3k +0.0082*x4k + 0.2703*angle - 0.0271*position; //controller utilise auparavant
...
/*x1=0.6300*x1k-0.1206*x2k-0.0008*x3k+0.0086*x4k+0.3658*angle+0.1200*position; // controller du prof
...
x4=-3.9680*x1k-1.7733*x2k-0.1546*x3k+0.7222*x4k+3.1377*angle+1.6599*position;*/
	...
	commande= (-80.3092*x1-9.6237*x2-14.1215*x3-23.6260*x4); // controller du prof et C calcule par nous meme
	//commande= (-104.7570*x1 -15.8810*x2 -20.5358*x3 -30.5773*x4);	// controller utilise auparavant
...
float conversionVoltToAngle(float angle){
		...
		printk("angle_converti = %d radians\n",(int)(angle_converti*1000.0/3.14*180.0));
